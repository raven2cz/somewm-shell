...
# Or one-liner:
./deploy.sh && kill $(pgrep -f 'qs -c somewm'); qs -c somewm -n -d &
```

**IMPORTANT:** Always edit in this repo, never in `~/.config/quickshell/somewm/`.
...
# Verbose output
bash tests/test-all.sh --verbose
```
...
- Add keybinding in `rc.lua` of [somewm-one](https://github.com/raven2cz/somewm-one)
  (override path with `SOMEWM_ONE_PATH`; default `~/git/github/somewm-one`)
...
# Deploy + restart QS
./deploy.sh && kill $(pgrep -f 'qs -c somewm')

# Deploy + clear cache + restart
./deploy.sh && rm -rf ~/.cache/quickshell/qmlcache/ && kill $(pgrep -f 'qs -c somewm')
